...
import { AXVMConstants } from "./constants"
 ...
 */
export class AXVMAPI extends JRPCAPI {
  /**
   * @ignore
   */
  ...
  refreshBlockchainID = (blockchainID: string = undefined): boolean => {
    ...
    if (
      ...
      this.blockchainID = Defaults.network[`${netid}`].X.blockchainID //default to AssetChain
      return true
    }
  ...
  getBalance = async (
    ...
    if (typeof this.parseAddress(address) === "undefined") {
      ...
      throw new AddressError(
        "Error - AXVMAPI.getBalance: Invalid address format"
      )
    }
  ...
  createNFTAsset = async (
    ...
    if (typeof changeAddr !== "undefined") {
      if (typeof this.parseAddress(changeAddr) === "undefined") {
        /* istanbul ignore next */
        throw new AddressError(
          "Error - AXVMAPI.createNFTAsset: Invalid address format"
        )
      }
  ...
  mintNFT = async (
    ...
    if (typeof this.parseAddress(to) === "undefined") {
      /* istanbul ignore next */
      throw new AddressError("Error - AXVMAPI.mintNFT: Invalid address format")
    }
    ...
    if (typeof changeAddr !== "undefined") {
      if (typeof this.parseAddress(changeAddr) === "undefined") {
        /* istanbul ignore next */
        throw new AddressError(
          "Error - AXVMAPI.mintNFT: Invalid address format"
        )
      }
      params["changeAddr"] = changeAddr
    }
  ...
  sendNFT = async (
    ...
    if (typeof this.parseAddress(to) === "undefined") {
      /* istanbul ignore next */
      throw new AddressError("Error - AXVMAPI.sendNFT: Invalid address format")
    }
    ...
    if (typeof changeAddr !== "undefined") {
      if (typeof this.parseAddress(changeAddr) === "undefined") {
        /* istanbul ignore next */
        throw new AddressError(
          "Error - AXVMAPI.sendNFT: Invalid address format"
        )
      }
      params["changeAddr"] = changeAddr
    }
  ...
  exportKey = async (
    ...
    if (typeof this.parseAddress(address) === "undefined") {
      /* istanbul ignore next */
      throw new AddressError(
        "Error - AXVMAPI.exportKey: Invalid address format"
      )
    }
  ...
  getAllBalances = async (address: string): Promise<object[]> => {
    if (typeof this.parseAddress(address) === "undefined") {
      /* istanbul ignore next */
      throw new AddressError(
        "Error - AXVMAPI.getAllBalances: Invalid address format"
      )
    }
  ...
  buildBaseTx = async (
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      ...
      throw new GooseEggCheckError(
        "Error - AXVMAPI.buildBaseTx:Failed Goose Egg Check"
      )
    }
  ...
  buildNFTTransferTx = async (
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - AXVMAPI.buildNFTTransferTx:Failed Goose Egg Check"
      )
    }
  ...
  buildImportTx = async (
    ...
    if (typeof sourceChain === "undefined") {
      throw new ChainIdError(
        "Error - AXVMAPI.buildImportTx: Source ChainID is undefined."
      )
      ...
      throw new ChainIdError(
        "Error - AXVMAPI.buildImportTx: Invalid destinationChain type: " +
          typeof sourceChain
      )
    }
    ...
    if (atomics.length === 0) {
      throw new NoAtomicUTXOsError(
        "Error - AXVMAPI.buildImportTx: No atomic UTXOs to import from " +
          srappChain +
          " using addresses: " +
          ownerAddresses.join(", ")
      )
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - AXVMAPI.buildImportTx:Failed Goose Egg Check"
      )
    }
  ...
  buildExportTx = async (
    ...
    if (Object.keys(prefixes).length !== 1) {
      throw new AddressError(
        "Error - AXVMAPI.buildExportTx: To addresses must have the same chainID prefix."
      )
    }

    if (typeof destinationChain === "undefined") {
      throw new ChainIdError(
        "Error - AXVMAPI.buildExportTx: Destination ChainID is undefined."
      )
    } else if (typeof destinationChain === "string") {
      destinationChain = bintools.cb58Decode(destinationChain) //
    } else if (!(destinationChain instanceof Buffer)) {
      throw new ChainIdError(
        "Error - AXVMAPI.buildExportTx: Invalid destinationChain type: " +
          typeof destinationChain
      )
    }
    if (destinationChain.length !== 32) {
      throw new ChainIdError(
        "Error - AXVMAPI.buildExportTx: Destination ChainID must be 32 bytes in length."
      )
    }
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - AXVMAPI.buildExportTx:Failed Goose Egg Check"
      )
    }
  ...
  buildCreateAssetTx = async (
    ...
    if (symbol.length > AXVMConstants.SYMBOLMAXLEN) {
      throw new SymbolError(
        "Error - AXVMAPI.buildCreateAssetTx: Symbols may not exceed length of " +
          AXVMConstants.SYMBOLMAXLEN
      )
    }
    if (name.length > AXVMConstants.ASSETNAMELEN) {
      throw new NameError(
        "Error - AXVMAPI.buildCreateAssetTx: Names may not exceed length of " +
          AXVMConstants.ASSETNAMELEN
      )
    }
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx, fee))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - AXVMAPI.buildCreateAssetTx:Failed Goose Egg Check"
      )
    }
  ...
  buildSECPMintTx = async (
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - AXVMAPI.buildSECPMintTx:Failed Goose Egg Check"
      )
    }
    return builtUnsignedTx
  }
  ...
  buildCreateNFTAssetTx = async (
    ...
    if (name.length > AXVMConstants.ASSETNAMELEN) {
      /* istanbul ignore next */
      throw new NameError(
        "Error - AXVMAPI.buildCreateNFTAssetTx: Names may not exceed length of " +
          AXVMConstants.ASSETNAMELEN
      )
    }
    if (symbol.length > AXVMConstants.SYMBOLMAXLEN) {
      /* istanbul ignore next */
      throw new SymbolError(
        "Error - AXVMAPI.buildCreateNFTAssetTx: Symbols may not exceed length of " +
          AXVMConstants.SYMBOLMAXLEN
      )
    }
    const networkID: number = this.core.getNetworkID()
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx, creationTxFee))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - AXVMAPI.buildCreateNFTAssetTx:Failed Goose Egg Check"
      )
    }
    return builtUnsignedTx
  }
  ...
  buildCreateNFTMintTx = async (
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - AXVMAPI.buildCreateNFTMintTx:Failed Goose Egg Check"
      )
    }
    return builtUnsignedTx
  }
  ...
  issueTx = async (tx: string | Buffer | Tx): Promise<string> => {
    ...
    if (typeof tx === "string") {
      ...
      throw new TransactionError(
        "Error - AXVMAPI.issueTx: provided tx is not expected type of string, Buffer, or Tx"
      )
    }
  ...
  send = async (
    ...
    if (typeof this.parseAddress(to) === "undefined") {
      /* istanbul ignore next */
      throw new AddressError("Error - AXVMAPI.send: Invalid address format")
    }
    ...
    sendOutputs.forEach(
      (output: {
        ...
        if (typeof this.parseAddress(output.to) === "undefined") {
          /* istanbul ignore next */
          throw new AddressError(
            "Error - AXVMAPI.sendMultiple: Invalid address format"
          )
        }
      ...
      for (let i: number = 0; i < addresses.length; i++) {
        if (typeof addresses[`${i}`] === "string") {
          if (
            ...
            /* istanbul ignore next */
            throw new AddressError(
              "Error - AXVMAPI.${caller}: Invalid address format"
            )
          }
