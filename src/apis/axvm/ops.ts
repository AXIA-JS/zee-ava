...
export const SelectOperationClass = (
  ...
  if (
    opid === AXVMConstants.SECPMINTOPID ||
    opid === AXVMConstants.SECPMINTOPID_CODECONE
  ) {
    return new SECPMintOperation(...args)
  } else if (
    opid === AXVMConstants.NFTMINTOPID ||
    opid === AXVMConstants.NFTMINTOPID_CODECONE
  ) {
    return new NFTMintOperation(...args)
  } else if (
    opid === AXVMConstants.NFTXFEROPID ||
    opid === AXVMConstants.NFTXFEROPID_CODECONE
  ) {
...
export class TransferableOperation extends Serializable {
  ...
  constructor(
    ...
    if (
      ...
      assetID.length === AXVMConstants.ASSETIDLEN &&
...
export class SECPMintOperation extends Operation {
  ...
  protected _typeID =
    this._codecID === 0
      ? AXVMConstants.SECPMINTOPID
      : AXVMConstants.SECPMINTOPID_CODECONE
  ...
  getCredentialID(): number {
    if (this._codecID === 0) {
      return AXVMConstants.SECPCREDENTIAL
    } else if (this._codecID === 1) {
      return AXVMConstants.SECPCREDENTIAL_CODECONE
    }
  }
...
export class NFTMintOperation extends Operation {
  ...
  protected _typeID =
    this._codecID === 0
      ? AXVMConstants.NFTMINTOPID
      : AXVMConstants.NFTMINTOPID_CODECONE
  ...
  getCredentialID = (): number => {
    if (this._codecID === 0) {
      return AXVMConstants.NFTCREDENTIAL
    } else if (this._codecID === 1) {
      return AXVMConstants.NFTCREDENTIAL_CODECONE
    }
  }
...
export class NFTTransferOperation extends Operation {
  ...
  protected _typeID =
    this._codecID === 0
      ? AXVMConstants.NFTXFEROPID
      : AXVMConstants.NFTXFEROPID_CODECONE
