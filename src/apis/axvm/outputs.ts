...
export const SelectOutputClass = (outputid: number, ...args: any[]): Output => {
  if (
    outputid === AXVMConstants.SECPXFEROUTPUTID ||
    outputid === AXVMConstants.SECPXFEROUTPUTID_CODECONE
  ) {
    return new SECPTransferOutput(...args)
  } else if (
    outputid === AXVMConstants.SECPMINTOUTPUTID ||
    outputid === AXVMConstants.SECPMINTOUTPUTID_CODECONE
  ) {
    return new SECPMintOutput(...args)
  } else if (
    outputid === AXVMConstants.NFTMINTOUTPUTID ||
    outputid === AXVMConstants.NFTMINTOUTPUTID_CODECONE
  ) {
    return new NFTMintOutput(...args)
  } else if (
    outputid === AXVMConstants.NFTXFEROUTPUTID ||
    outputid === AXVMConstants.NFTXFEROUTPUTID_CODECONE
  ) {
...
export class TransferableOutput extends StandardTransferableOutput {
  ...
  fromBuffer(bytes: Buffer, offset: number = 0): number {
    this.assetID = bintools.copyFrom(
      bytes,
      offset,
      offset + AXVMConstants.ASSETIDLEN
    )
    offset += AXVMConstants.ASSETIDLEN
...
export class SECPTransferOutput extends AmountOutput {
  ...
  protected _codecID = AXVMConstants.LATESTCODEC
  protected _typeID =
    this._codecID === 0
      ? AXVMConstants.SECPXFEROUTPUTID
      : AXVMConstants.SECPXFEROUTPUTID_CODECONE
...
export class SECPMintOutput extends Output {
  ...
  protected _typeID =
    this._codecID === 0
      ? AXVMConstants.SECPMINTOUTPUTID
      : AXVMConstants.SECPMINTOUTPUTID_CODECONE
...
export class NFTMintOutput extends NFTOutput {
  ...
  protected _typeID =
    this._codecID === 0
      ...
      : AXVMConstants.NFTMINTOUTPUTID_CODECONE
...
export class NFTTransferOutput extends NFTOutput {
  ...
  protected _typeID =
    this._codecID === 0
      ? AXVMConstants.NFTXFEROUTPUTID
      : AXVMConstants.NFTXFEROUTPUTID_CODECONE
