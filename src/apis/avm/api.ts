...
import { Defaults, PlatformChainID, PrimaryAssetAlias, ONEAXC } from '../../utils/constants';
...
export class AVMAPI extends JRPCAPI {
  ...
  checkGooseEgg = async (utx:UnsignedTx, outTotal:BN = new BN(0)): Promise<boolean> => {
    ...
    let outputTotal:BN = outTotal.gt(new BN(0)) ? outTotal : utx.getOutputTotal(axcAssetID);
  ...
  /**
     ...
     * @returns String representing the transaction id
     */
  exportAXC = async (username:string, password:string, to:string, amount:BN):Promise<string> => {
    const params:any = {
      to,
      amount: amount.toString(10),
      username,
    ...
    return this.callMethod('avm.exportAXC', params).then((response:RequestResponseData) => response.data.result.txID);
  };
  ...
  /**
     ...
     * @returns String representing the transaction id
     */
  importAXC = async (username:string, password:string, to:string, sourceChain:string):Promise<string> => {
    const params:any = {
      to,
      sourceChain,
      username,
    ...
    return this.callMethod('avm.importAXC', params).then((response:RequestResponseData) => response.data.result.txID);
  };
  ...
  buildSECPMintTx = async (
    ...
    let axcAssetID:Buffer = await this.getAXCAssetID();
  ...
  */
   ...
   */
  constructor(core:AxiaCore, baseurl:string = '/ext/bc/X', blockchainID:string = '') {
    super(core, baseurl);
    this.blockchainID = blockchainID;
