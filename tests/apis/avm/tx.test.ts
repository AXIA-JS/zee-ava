...
describe('Transactions', () => {
  ...
  const memo:Buffer = Buffer.from("AxiaJS");
  ...
  beforeAll(async () => {
    axia = new Axia(ip, port, protocol, netid, undefined, undefined, null, true);
    api = new AVMAPI(axia, '/ext/bc/avm', blockchainid);
  ...
  test('Create small BaseTx that is Goose Egg Tx', async () => {
    ...
    const transferableOutput:TransferableOutput = new TransferableOutput(axcAssetID, output);
    ...
    const transferableInput:TransferableInput = new TransferableInput(txid, outputIndex, axcAssetID, input);
  ...
  test('Creation UnsignedTx Check Amount', () => {
    expect(() => {
      set.buildBaseTx(
        netid, blockchainID,
        ONEAXC.mul(new BN(amnt * 10000)), assetID,
  ...
  test('Creation Tx3 using OperationTx', () => {
    const txu:UnsignedTx = set.buildNFTTransferTx(
      netid, blockchainID, 
      addrs3, addrs1, addrs2, nftutxoids, new BN(90), axcAssetID, undefined,
      UnixNow(), UnixNow().add(new BN(50)), 1,
    );
    const tx:Tx = txu.sign(keymgr1);
    const tx2:Tx = new Tx();
  ...
  test('Creation Tx5 using ExportTx', () => {
    const txu:UnsignedTx = set.buildExportTx(
      netid, blockchainID, new BN(90), axcAssetID,
