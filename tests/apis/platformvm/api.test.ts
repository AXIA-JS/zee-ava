...
describe('PlatformVMAPI', () => {
  ...
  const axia:Axia = new Axia(ip, port, protocol, networkid, undefined, undefined, undefined, true);
  let api:PlatformVMAPI;
  let alias:string;

  const addrA:string = 'P-' + bech32.encode(axia.getHRP(), bech32.toWords(bintools.cb58Decode("B6D4v1VtPYLbiUvYXtW4Px8oE9imC2vGW")));
  const addrB:string = 'P-' + bech32.encode(axia.getHRP(), bech32.toWords(bintools.cb58Decode("P5wdRuZeaDt28eHMP5S3w9ZdoBfo7wuzF")));
  const addrC:string = 'P-' + bech32.encode(axia.getHRP(), bech32.toWords(bintools.cb58Decode("6Y3kysjF9jnHnYkdS9yGAuoHyae2eNmeV")));
  ...
  test("exportAXC", async ()=>{
    let amount = new BN(100);
    let to = "abcdef";
    let username = "Robert";
    let password = "Paulson";
    let txID = "valid";
    let result:Promise<string> = api.exportAXC(username, password, amount, to);
    let payload:object = {
        "result": {
            "txID": txID
        }
  ...
  test("importAXC", async ()=>{
    let to = "abcdef";
    let username = "Robert";
    let password = "Paulson";
    let txID = "valid";
    let result:Promise<string> = api.importAXC(username, password, to, blockchainid);
    let payload:object = {
        "result": {
            "txID": txID
        }
  ...
  describe('Transactions', () => {
    ...
    beforeEach(async () => {
      ...
      for (let i:number = 0; i < 4; i++) {
        ...
        const out:SECPTransferOutput = new SECPTransferOutput(ONEAXC.mul(new BN(5)), addressbuffs, locktime, 1);
        const pout:ParseableOutput = new ParseableOutput(out);
        const lockout:StakeableLockOut = new StakeableLockOut(ONEAXC.mul(new BN(5)), addressbuffs, locktime, 1, locktime.add(new BN(86400)), pout);
    ...
    test('buildImportTx', async () => {
      ...
      const txu2:UnsignedTx = set.buildImportTx(
        ...
        addrbuff3, addrbuff1, addrbuff2, [fungutxo], bintools.cb58Decode(PlatformChainID), platformvm.getTxFee(), await platformvm.getAXCAssetID(), 
    ...
    test('buildExportTx', async () => {
      ...
      const txu1:UnsignedTx = await platformvm.buildExportTx(
        set, 
        amount,
        bintools.cb58Decode(Defaults.network[axia.getNetworkID()].X["blockchainID"]),
        addrbuff3.map((a) => bintools.addressToString(axia.getHRP(), "P", a)), 
        addrs1, 
        addrs2,
      ...
      const txu3:UnsignedTx = await platformvm.buildExportTx(
        set, amount, bintools.cb58Decode(Defaults.network[axia.getNetworkID()].X["blockchainID"]), 
...
*/
    ...
    test('buildAddValidatorTx 3', async () => {
      ...
      let lockedBaseOut:SECPTransferOutput = new SECPTransferOutput(ONEAXC.mul(new BN(2)), addrbuff1, locktime, 1);
      let lockedBaseXOut:ParseableOutput = new ParseableOutput(lockedBaseOut);
      let lockedOut:StakeableLockOut = new StakeableLockOut(ONEAXC.mul(new BN(2)), addrbuff1, locktime, 1, locktime, lockedBaseXOut)
      ...
      let unlockedOut:SECPTransferOutput = new SECPTransferOutput(ONEAXC.mul(new BN(2)), addrbuff1, locktime, 1);
