...
import { KeyPair, KeyChain } from "../../../src/apis/axvm/keychain"
import { Buffer } from "buffer/"
import BN from "bn.js"
import BinTools from "../../../src/utils/bintools"
import { UTXOSet, UTXO } from "../../../src/apis/axvm/utxos"
import {
  TransferableInput,
  SECPTransferInput
} from "../../../src/apis/axvm/inputs"
import createHash from "create-hash"
import { UnsignedTx, Tx } from "../../../src/apis/axvm/tx"
import { AXVMConstants } from "../../../src/apis/axvm/constants"
import {
  TransferableOutput,
  SECPTransferOutput,
  NFTMintOutput,
  NFTTransferOutput,
  SECPMintOutput
} from "../../../src/apis/axvm/outputs"
import {
  NFTTransferOperation,
  TransferableOperation,
  SECPMintOperation
} from "../../../src/apis/axvm/ops"
...
import { MinterSet } from "../../../src/apis/axvm/minterset"
...
const serialzeit = (aThing: Serializable, name: string): void => {
  if (dumpSerailization) {
    console.log(
      JSON.stringify(
        serialization.serialize(aThing, "axvm", "hex", name + " -- Hex Encoded")
      )
    )
    console.log(
      JSON.stringify(
...
describe("AXVMAPI", (): void => {
  ...
  beforeAll((): void => {
    api = new AXVMAPI(axia, "/ext/bc/X", blockchainID)
  ...
  test("refreshBlockchainID", async (): Promise<void> => {
    ...
    const testAPI: AXVMAPI = new AXVMAPI(axia, "/ext/bc/axvm", n3bcID)
  ...
  test("getBalance includePartial", async (): Promise<void> => {
    ...
    const calledWith: object = {
      ...
      data: '{"id":9,"method":"axvm.getBalance","params":{"address":"X-custom1d6kkj0qh4wcmus3tk59npwt3rluc6en755a58g","assetID":"ATH","includePartial":true},"jsonrpc":"2.0"}',
      headers: {
  ...
  describe("Transactions", (): void => {
    ...
    beforeEach(async (): Promise<void> => {
      axvm = new AXVMAPI(axia, "/ext/bc/X", blockchainID)

      const result: Promise<Buffer> = axvm.getAXCAssetID(true)
      const payload: object = {
        result: {
          name,
          symbol,
      ...
      for (let i: number = 0; i < 3; i++) {
        addrs1.push(
          axvm.addressFromBuffer(axvm.keyChain().makeKey().getAddress())
        )
        addrs2.push(axvm.addressFromBuffer(keymgr2.makeKey().getAddress()))
        addrs3.push(axvm.addressFromBuffer(keymgr3.makeKey().getAddress()))
      }
      const amount: BN = ONEAXC.mul(new BN(amnt))
      addressbuffs = axvm.keyChain().getAddresses()
      addresses = addressbuffs.map((a) => axvm.addressFromBuffer(a))
      ...
      secpbase1 = new SECPTransferOutput(
        new BN(777),
        addrs3.map((a) => axvm.parseAddress(a)),
        UnixNow(),
        1
      )
      secpbase2 = new SECPTransferOutput(
        new BN(888),
        addrs2.map((a) => axvm.parseAddress(a)),
        UnixNow(),
        1
      )
      ...
      initialState.addOutput(secpbase1, AXVMConstants.SECPFXID)
      initialState.addOutput(secpbase2, AXVMConstants.SECPFXID)
      initialState.addOutput(secpbase3, AXVMConstants.SECPFXID)
      ...
      nftInitialState.addOutput(nftpbase1, AXVMConstants.NFTFXID)
      nftInitialState.addOutput(nftpbase2, AXVMConstants.NFTFXID)
      nftInitialState.addOutput(nftpbase3, AXVMConstants.NFTFXID)
    ...
    test("getDefaultMintTxFee", (): void => {
      expect(axvm.getDefaultMintTxFee().toString()).toBe("1000000")
    })

    test("signTx", async (): Promise<void> => {
      const txu1: UnsignedTx = await axvm.buildBaseTx(
        set,
        new BN(amnt),
      ...
      const txu2: UnsignedTx = set.buildBaseTx(
        ...
        assetID,
        addrs3.map((a) => axvm.parseAddress(a)),
        addrs1.map((a) => axvm.parseAddress(a)),
        addrs1.map((a) => axvm.parseAddress(a)),
        axvm.getTxFee(),
        assetID,
        undefined,
      ...
      const tx1: Tx = axvm.signTx(txu1)
      const tx2: Tx = axvm.signTx(txu2)
    ...
    test("buildBaseTx1", async (): Promise<void> => {
      ...
      const tx1: Tx = txu1.sign(axvm.keyChain())
      ...
      const tx3: Tx = txu1.sign(axvm.keyChain())
    ...
    test("buildBaseTx2", async (): Promise<void> => {
      ...
      const txu2: UnsignedTx = set.buildBaseTx(
        ...
        new BN(amnt).sub(new BN(100)),
        assetID,
        addrs3.map((a): Buffer => axvm.parseAddress(a)),
        addrs1.map((a): Buffer => axvm.parseAddress(a)),
        addrs2.map((a): Buffer => axvm.parseAddress(a)),
        axvm.getTxFee(),
        assetID,
      ...
      const outaddr0 = outies[0]
        .getOutput()
        .getAddresses()
        .map((a) => axvm.addressFromBuffer(a))
    ...
    test("issueTx Serialized", async (): Promise<void> => {
      const txu: UnsignedTx = await axvm.buildBaseTx(
        set,
        new BN(amnt),
        bintools.cb58Encode(assetID),
        addrs3,
      ...
      const tx = axvm.signTx(txu)
      const txid: string =
        "f966750f438867c3c9828ddcdbe660e21ccdbb36a9276958f011ba472f75d4e7"

      const result: Promise<string> = axvm.issueTx(tx.toString())
      const payload: object = {
        result: {
          txID: txid
        }
    ...
    test("issueTx Buffer", async (): Promise<void> => {
      ...
      const txid: string =
        "f966750f438867c3c9828ddcdbe660e21ccdbb36a9276958f011ba472f75d4e7"
      const result: Promise<string> = axvm.issueTx(tx.toBuffer())
      const payload: object = {
        result: {
          txID: txid
        }
    ...
    test("issueTx Class Tx", async (): Promise<void> => {
      ...
      const result: Promise<string> = axvm.issueTx(tx)
      const payload: object = {
        result: {
          txID: txid
        }
    ...
    test("buildCreateAssetTx - Fixed Cap", async (): Promise<void> => {
      axvm.setCreationTxFee(new BN(fee))
      const txu1: UnsignedTx = await axvm.buildCreateAssetTx(
        set,
        addrs1,
        addrs2,
        initialState,
      ...
      const txu2: UnsignedTx = set.buildCreateAssetTx(
        axia.getNetworkID(),
        bintools.cb58Decode(axvm.getBlockchainID()),
        addrs1.map((a) => axvm.parseAddress(a)),
        addrs2.map((a) => axvm.parseAddress(a)),
        initialState,
        name,
    ...
    test("buildCreateAssetTx - Variable Cap", async (): Promise<void> => {
      axvm.setCreationTxFee(new BN(Defaults.network[12345].P["creationTxFee"]))
    ...
    test("buildSECPMintTx", async (): Promise<void> => {
      axvm.setTxFee(new BN(fee))
      const newMinter: SECPMintOutput = new SECPMintOutput(
        addrs3.map((a) => axvm.parseAddress(a)),
        new BN(0),
        1
      )
      const txu1: UnsignedTx = await axvm.buildSECPMintTx(
        set,
        newMinter,
        secpMintXferOut1,
        addrs1,
    ...
    test("buildCreateNFTAssetTx", async (): Promise<void> => {
      ...
      const txu1: UnsignedTx = await axvm.buildCreateNFTAssetTx(
        set,
        addrs1,
        addrs2,
        minterSets,
      ...
      const txu2: UnsignedTx = set.buildCreateNFTAssetTx(
        axia.getNetworkID(),
        bintools.cb58Decode(axvm.getBlockchainID()),
        addrs1.map((a: string): Buffer => axvm.parseAddress(a)),
        addrs2.map((a: string): Buffer => axvm.parseAddress(a)),
        minterSets,
        name,
        symbol,
        axvm.getCreationTxFee(),
    ...
    test("buildCreateNFTMintTx", async (): Promise<void> => {
      ...
      const addrbuff1: Buffer[] = addrs1.map(
        (a: string): Buffer => axvm.parseAddress(a)
      )
      ...
      const txu1: UnsignedTx = await axvm.buildCreateNFTMintTx(
        set,
        oo,
        addrs1,
        addrs2,
      ...
      const txu3: UnsignedTx = await axvm.buildCreateNFTMintTx(
        set,
        outputOwners,
        addrs1,
        addrs2,
    ...
    test("buildNFTTransferTx", async (): Promise<void> => {
      ...
      const addrbuff1 = addrs1.map((a: string): Buffer => axvm.parseAddress(a))
      const addrbuff2 = addrs2.map((a: string): Buffer => axvm.parseAddress(a))
      const addrbuff3 = addrs3.map((a: string): Buffer => axvm.parseAddress(a))
      const txu1: UnsignedTx = await axvm.buildNFTTransferTx(
        set,
        addrs3,
        addrs1,
        addrs2,
    ...
    test("buildImportTx", async (): Promise<void> => {
      ...
      const threshold: number = 1
      axvm.setTxFee(new BN(fee))
      const addrbuff1 = addrs1.map((a) => axvm.parseAddress(a))
      const addrbuff2 = addrs2.map((a) => axvm.parseAddress(a))
      const addrbuff3 = addrs3.map((a) => axvm.parseAddress(a))
      const fungutxo: UTXO = set.getUTXO(fungutxoids[1])
      const fungutxostr: string = fungutxo.toString()

      const result: Promise<UnsignedTx> = axvm.buildImportTx(
        set,
        addrs1,
        PlatformChainID,
        addrs3,
    ...
    test("buildExportTx", async (): Promise<void> => {
      ...
      const txu1: UnsignedTx = await axvm.buildExportTx(
        set,
        amount,
        bintools.cb58Decode(PlatformChainID),
      ...
      const txu3: UnsignedTx = await axvm.buildExportTx(
        set,
        amount,
        PlatformChainID,
        addrs3,
